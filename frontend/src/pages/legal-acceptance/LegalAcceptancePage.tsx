...
const LegalAcceptancePage = () => {
  ...
  useEffect(() => {
    if (pendingDocs.length === 0 && !isLoading) {
      const redirectPath = sessionStorageUtils.getRedirectAfterLegal() || '/home';
      sessionStorageUtils.clearRedirectAfterLegal();
      navigate(redirectPath, { replace: true });
    }
  }, [pendingDocs.length, isLoading, navigate]);
      ...
      <Container maxWidth="md" className="page-container">
        <Paper className="acceptance-paper">
          ...
          <Stack className="document-section">
            ...
            <Typography variant="body2" color="text.secondary">
              {t('VERSION')}: {currentDoc.version} | {t('EFFECTIVE')}: {format(new Date(currentDoc.effectiveAt), 'dd.MM.yyyy')}
            </Typography>
